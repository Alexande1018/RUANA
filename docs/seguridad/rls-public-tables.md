...
**Estado:** migración lista y se aplica en producción vía GitHub Actions (`.github/workflows/apply-rls-public.yml` y paso idempotente en `deploy-firebase.yml`).  
**Migración:** `supabase/migrations/20260902000200_enable_rls_public_tables.sql`  
**Fecha inventario:** 2026-09-02 · **Aplicación autorizada:** 2026-09-04
...
No activar `FORCE ROW LEVEL SECURITY` (rompería Flask, que usa `DATABASE_URL` como dueño de tabla). La aplicación en prod se hace con el script idempotente, no pegando SQL a mano.
