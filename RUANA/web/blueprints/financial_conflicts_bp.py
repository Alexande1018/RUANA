...
Rutas duales (estilo RUANA): inglés + español.
"""
...
_BASE_EN = "/api/admin/financial-conflicts"
_BASE_ES = "/api/admin/conflictos-financieros"
...
@financial_conflicts_bp.route(f"{_BASE_EN}/bp-health", methods=["GET"])
@financial_conflicts_bp.route(f"{_BASE_ES}/bp-health", methods=["GET"])
def financial_conflicts_bp_health():
    return jsonify({"status": "ok", "dominio": "financial_conflicts"})


@financial_conflicts_bp.route(_BASE_EN, methods=["GET"])
@financial_conflicts_bp.route(_BASE_ES, methods=["GET"])
...
@financial_conflicts_bp.route(f"{_BASE_EN}/<int:conflict_id>", methods=["GET"])
@financial_conflicts_bp.route(f"{_BASE_ES}/<int:conflict_id>", methods=["GET"])
@require_conflict_permission(CONFLICT_VIEW)
def detalle_conflicto(conflict_id: int):
    """GET /api/admin/financial-conflicts/<conflict_id> — detalle completo."""
    try:
...
@financial_conflicts_bp.route(
    f"{_BASE_EN}/<int:conflict_id>/solicitar-evidencia", methods=["POST"],
)
@financial_conflicts_bp.route(
...
@financial_conflicts_bp.route(f"{_BASE_EN}/<int:conflict_id>/auditoria", methods=["GET"])
@financial_conflicts_bp.route(f"{_BASE_ES}/<int:conflict_id>/auditoria", methods=["GET"])
@require_conflict_permission(CONFLICT_VIEW)
...
@financial_conflicts_bp.route(
    f"{_BASE_EN}/<int:conflict_id>/acciones-pendientes", methods=["GET"],
)
@financial_conflicts_bp.route(
    f"{_BASE_ES}/<int:conflict_id>/acciones-pendientes", methods=["GET"],
)
@require_conflict_permission(CONFLICT_VIEW)
