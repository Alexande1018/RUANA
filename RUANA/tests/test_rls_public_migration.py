...
APPLY_SCRIPT = ROOT / ".github" / "scripts" / "apply_rls_public_tables.py"
APPLY_WORKFLOW = ROOT / ".github" / "workflows" / "apply-rls-public.yml"
...
def test_rls_apply_script_and_workflows_existen():
    assert APPLY_SCRIPT.is_file()
    script = APPLY_SCRIPT.read_text(encoding="utf-8")
    assert "20260902000200_enable_rls_public_tables.sql" in script
    assert APPLY_WORKFLOW.is_file()
    wf = APPLY_WORKFLOW.read_text(encoding="utf-8")
    assert "apply_rls_public_tables.py" in wf
    deploy = DEPLOY_WORKFLOW.read_text(encoding="utf-8")
    assert "apply_rls_public_tables.py" in deploy
