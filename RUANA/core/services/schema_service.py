...
def _migrar_grupo_madre_v1_postgres(cursor) -> None:
    """DDL idempotente de Grupo Madre para Supabase/Postgres (sin PRAGMA ni AUTOINCREMENT)."""
    cursor.execute(
        "ALTER TABLE IF EXISTS public.grupos ADD COLUMN IF NOT EXISTS tipo TEXT DEFAULT 'territorial'"
    )
    cursor.execute(
        "ALTER TABLE IF EXISTS public.grupos ADD COLUMN IF NOT EXISTS grupo_madre_id BIGINT REFERENCES public.grupos(id)"
    )
    cursor.execute(
        "UPDATE public.grupos SET tipo = 'territorial' WHERE tipo IS NULL OR btrim(tipo) = ''"
    )
    cursor.execute("""
    ...
    """)
    cursor.execute("""
        INSERT INTO public.cp_estado (codigo_postal, ciudad, modo)
...
def _migrar_grupo_madre_v1_si_procede(db, conn, cursor) -> None:
    ...
    if getattr(db, "backend", None) == "postgres":
        _migrar_grupo_madre_v1_postgres(cursor)
...
def _migrar_cp_auto_split_v1_si_procede(db, conn, cursor) -> None:
    ...
    if getattr(db, "backend", None) == "postgres":
        _repo.execute(
            cursor,
            "ALTER TABLE cp_estado ADD COLUMN IF NOT EXISTS aliados_desde_ultimo_grupo INTEGER NOT NULL DEFAULT 0",
        )
...
def _migrar_competencia_postgres(db, cursor) -> None:
    ...
    cursor.execute("""
        DO $$
        BEGIN
            IF to_regclass('public.competencia') IS NOT NULL THEN
                IF EXISTS (
...
def _migrar_payment_conflicts_postgres(db, cursor) -> None:
    ...
    """)
    cursor.execute("ALTER TABLE public.payment_conflicts ADD COLUMN IF NOT EXISTS tipo TEXT DEFAULT 'importe_discrepante'")
...
def _asegurar_tabla_migraciones(db, cursor) -> None:
    """Tabla de control de migraciones (SQLite y Postgres)."""
    if getattr(db, "backend", None) == "postgres":
        cursor.execute(
            """
            CREATE TABLE IF NOT EXISTS migraciones (
...
def _init_postgres_schema(db):
    ...
    try:
        ...
        db._migrar_privacidad_rgpd_aliado(conn, cursor)
        db._migrar_stripe_pagos(conn, cursor)
        _migrar_competencia_postgres(db, cursor)
        _migrar_payment_conflicts_postgres(db, cursor)
        _asegurar_tabla_migraciones(db, cursor)
        try:
            db._migrar_grupo_madre_v1_si_procede(conn, cursor)
            conn.commit()
        except Exception as madre_schema_exc:
            try:
                conn.rollback()
            except Exception:
                pass
            _log_schema_init_failed(madre_schema_exc)
        try:
            db._migrar_cp_auto_split_v1_si_procede(conn, cursor)
            conn.commit()
            ...
            try:
                ...
                pass
            print(f"[RUANA][DB] Aviso cp_auto_split: {split_exc}")
        try:
            db._migrar_grupo_madre_v2_consolidar_si_procede(conn, cursor)
            conn.commit()
