    ...
    with db._lock:
        try:
            ...
            if not row or not row[0]:
                return None
            grupo_id, oficio, score_actual = row[0], row[1], int(row[2] or 0)
            codigo_postal = (row[3] or "").strip()
            ciudad, provincia = row[4], row[5]
            grupo_tipo = (row[6] or "territorial").strip() if len(row) > 6 else "territorial"
            if not oficio or not codigo_postal:
                return None
            if db.competencia_activa_para_grupo_oficio(grupo_id, oficio):
                return None
            retador = _buscar_retador(
                db, codigo_aliado, grupo_id, oficio, score_actual, codigo_postal,
...
def _avisar_grupos_cp_competencia(
    ...
    try:
        cursor.execute(
            ...
            WHERE TRIM(codigo_postal) = ? AND estado IN ('activo', 'en_competencia')
            """,
            (cp,),
        )
